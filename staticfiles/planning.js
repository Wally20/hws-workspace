(() => {
  ...
  const slidesContainer = editorForm?.querySelector("[data-planning-slides]");
  ...
  const refreshSlides = () => {
    const slides = [...(slidesContainer?.querySelectorAll(":scope > [data-planning-slide]") || [])];
    let dayNumber = 0;
    let bulletNumber = 0;
    slides.forEach((slide, slideIndex) => {
      const slideType = slide.dataset.planningSlideType;
      const label = slide.querySelector("[data-planning-slide-label]");
      const dragHandle = slide.querySelector("[data-planning-slide-drag]");
      let typeLabel = "Taakverdeling";
      if (slideType === "day") {
        dayNumber += 1;
        typeLabel = `Dagplanning ${dayNumber}`;
      } else if (slideType === "bulletPoints") {
        bulletNumber += 1;
        typeLabel = `Bulletpointslide ${bulletNumber}`;
      }
      if (label) {
        label.textContent = `Slide ${slideIndex + 1} · ${typeLabel}`;
      }
      if (dragHandle) {
        dragHandle.setAttribute("aria-label", `Sleep ${typeLabel.toLowerCase()} vanaf positie ${slideIndex + 1}`);
      }
    });
    const days = slides.filter((slide) => slide.dataset.planningSlideType === "day");
    days.forEach((day) => {
  ...
  const resetSlideDragState = () => {
    slidesContainer?.querySelectorAll(".is-slide-drop-target, .is-slide-dragging, .is-touch-dragging").forEach((slide) => {
      slide.classList.remove("is-slide-drop-target", "is-slide-dragging", "is-touch-dragging");
    });
    draggedSlide = null;
    refreshSlides();
  };

  const moveSlideByKeyboard = (slide, direction) => {
    if (!slidesContainer || slide?.parentElement !== slidesContainer) {
      return;
    }
    ...
    if (direction < 0) {
      slidesContainer.insertBefore(slide, sibling);
    } else {
      slidesContainer.insertBefore(sibling, slide);
    }
    refreshSlides();
    slide.querySelector("[data-planning-slide-drag]")?.focus();
  };

  const bindSlidesContainer = () => {
    if (!slidesContainer) {
      return;
    }

    slidesContainer.addEventListener("keydown", (event) => {
      const handle = event.target.closest("[data-planning-slide-drag]");
      if (!handle || !["ArrowUp", "ArrowDown"].includes(event.key)) {
        return;
      }
      event.preventDefault();
      moveSlideByKeyboard(handle.closest("[data-planning-slide]"), event.key === "ArrowUp" ? -1 : 1);
    });
    ...
    slidesContainer.addEventListener("dragstart", (event) => {
      ...
      draggedSlide = handle.closest("[data-planning-slide]");
      draggedSlide?.classList.add("is-slide-dragging");
      if (event.dataTransfer) {
        event.dataTransfer.effectAllowed = "move";
        event.dataTransfer.setData("text/plain", "planning-slide");
      }
    });

    slidesContainer.addEventListener("dragover", (event) => {
      if (!draggedSlide || draggedSlide.parentElement !== slidesContainer) {
        return;
      }
      event.preventDefault();
      const targetSlide = event.target.closest("[data-planning-slide]");
      slidesContainer.querySelectorAll(".is-slide-drop-target").forEach((slide) => slide.classList.remove("is-slide-drop-target"));
      if (!targetSlide || targetSlide === draggedSlide || targetSlide.parentElement !== slidesContainer) {
        return;
      }
      targetSlide.classList.add("is-slide-drop-target");
      const targetRect = targetSlide.getBoundingClientRect();
      const insertAfter = event.clientY > targetRect.top + targetRect.height / 2;
      slidesContainer.insertBefore(draggedSlide, insertAfter ? targetSlide.nextElementSibling : targetSlide);
    });

    slidesContainer.addEventListener("drop", (event) => {
      if (!draggedSlide) {
        return;
      }
      event.preventDefault();
    ...
    slidesContainer.addEventListener("dragend", () => {
      if (draggedSlide) {
        resetSlideDragState();
      }
    });
    ...
    slidesContainer.addEventListener("pointerdown", (event) => {
      ...
      draggedSlide = handle.closest("[data-planning-slide]");
      draggedSlide?.classList.add("is-slide-dragging", "is-touch-dragging");
      handle.setPointerCapture?.(event.pointerId);
      event.preventDefault();
    });

    slidesContainer.addEventListener("pointermove", (event) => {
      if (!draggedSlide || event.pointerType === "mouse") {
        return;
      }
      event.preventDefault();
      const otherSlides = [...slidesContainer.querySelectorAll(":scope > [data-planning-slide]")]
        .filter((slide) => slide !== draggedSlide);
      const nextSlide = otherSlides.find((slide) => {
        const rect = slide.getBoundingClientRect();
        return event.clientY < rect.top + rect.height / 2;
      });
      if (nextSlide) {
        slidesContainer.insertBefore(draggedSlide, nextSlide);
      } else {
        slidesContainer.append(draggedSlide);
      }
    }, { passive: false });
    ...
    slidesContainer.addEventListener("pointerup", finishPointerDrag);
    slidesContainer.addEventListener("pointercancel", finishPointerDrag);
  };
  ...
  const countPrecedingDays = (slide) => {
    let count = 0;
    let sibling = slide?.previousElementSibling;
    while (sibling) {
      if (sibling.dataset.planningSlideType === "day") {
        count += 1;
      }
  ...
  const collectTaskAssignment = () => ({
    ...
    insertAfterDay: countPrecedingDays(taskEnabledInput?.closest("[data-planning-slide]")),
  ...
  const collectBulletSlides = () =>
    [...(bulletSlidesContainer?.querySelectorAll("[data-planning-bullet-slide]") || [])].map((slide) => ({
      ...
      insertAfterDay: countPrecedingDays(slide),
  ...
  const collectSlideOrder = () => {
    let dayIndex = 0;
    let bulletIndex = 0;
    return [...(slidesContainer?.querySelectorAll(":scope > [data-planning-slide]") || [])].map((slide) => {
      const slideType = slide.dataset.planningSlideType;
      if (slideType === "day") {
        const item = { type: slideType, index: dayIndex };
        dayIndex += 1;
        return item;
      }
      if (slideType === "bulletPoints") {
        const item = { type: slideType, index: bulletIndex };
        bulletIndex += 1;
        return item;
      }
  ...
  editorForm?.addEventListener("submit", () => {
    ...
    const slideOrderField = editorForm.querySelector("[data-planning-slide-order-json]");
    if (slideOrderField) {
      slideOrderField.value = JSON.stringify(collectSlideOrder());
    }
  });
